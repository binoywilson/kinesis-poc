...
export async function startMasterChannel(
  ...
  master.signalingClient.on('sdpOffer', async (offer, remoteClientId) => {
    ...
    peerConnection.addEventListener('track', (event) => {
      ...
      if (remoteView) {
        $(remoteView).removeClass('d-none');
      }

      let childDiv = $(remoteView).find(`div #${remoteClientId}`)[0];
      if (childDiv) {
        let videoChild = childDiv.querySelector('video');
        if (videoChild) {
          if (videoChild.srcObject) {
            return;
          }
          videoChild.srcObject = event.streams[0];
        }
      } else {
        let div = document.createElement('div');
        div.classList.add('col-4');
        div.id = remoteClientId;
        ...
        pTag.innerHTML = remoteClientId;

        div.appendChild(pTag);

        remoteView.appendChild(div);
      }
...
export function stopMasterChannel() {
...
export function sendMasterMessageChannel(message) {
